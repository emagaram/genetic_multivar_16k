...
from settings import FINGER_GOAL_GRID, FINGER_MAX_GRID, MODE, InaccuracyMode
...
def get_score_stats(kb: Keyboard, performance: dict[str, float]):
    ...
    for stat_name, raw_score in performance.items():
        res += f"{space}{stat_name}: {raw_score}\n"
    res += "\n"
    ...
    for i, hand in enumerate(finger_freqs):
        for j, freq in enumerate(hand):
            ...
            res += f"{space}{side} {finger}: {freq*100:.3f}%"
            if settings.FINGER_FREQ_WEIGHT > 0:
                res += f", Max: {FINGER_MAX_GRID[i][j]*100:.3f}%"
            res += "\n"
    res += "\n"
    res += "Row Usage:\n"
    characters = get_characters()
    chars_freq = sum(characters.values())
    for i in range(len(kb.keyboard[0][0])):
        row_pct = (
            sum(
                characters[char]
                for hand in kb.keyboard
                for finger in hand
                for char in finger[i].letters
            )
            / chars_freq
        )
        res += f"{space}Row {i}: {row_pct*100}%\n"
    res += "\n"

    # SFB
    sfb_evaluator = SFBSFSEvaluator(kb)
    sfb_sum_only1u = sum(
        sfb_evaluator.evaluate_bigram_stat(
            bigram, True, sfb_evaluator.corpus_frequencies.bigrams_freq
        )
    ...
    sfb_sum = sum(
        sfb_evaluator.evaluate_bigram_stat(
            bigram, False, sfb_evaluator.corpus_frequencies.bigrams_freq
        )
        for bigram in sfb_evaluator.bigrams.items()
    )
    ...
    for i, skipgrams in enumerate(sfb_evaluator.skipgrams):
        sfs_sum = sum(
            sfb_evaluator.evaluate_skipgram_stat(skipgram, True, i)
            for skipgram in skipgrams.items()
        )
    ...
    for i, skipgrams in enumerate(sfb_evaluator.skipgrams):
        sfs_sum = sum(
            sfb_evaluator.evaluate_skipgram_stat(skipgram, False, i)
            for skipgram in skipgrams.items()
        )
    ...
    discomfort_sum = sum(
        discomfort_evaluator.evaluate_bigram_stat(bigram)
        for bigram in sfb_evaluator.bigrams.items()
    )
