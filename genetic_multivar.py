...
def get_rank_inverse(target_rank: float, score_list: list[float]) -> float:
    index = math.ceil(len(score_list) * target_rank)
...
def calculate_kb_score(
    ...
    if kb not in scores_cache:
        ...
        for trigram in redirect_evaluator.trigrams.items():
            ...
            if (
                score
                + constants.REDIRECT_WEIGHT
                * get_rank(redirect_sum, scores_dict[Categories.REDIRECT])
                > best
            ):
                full_score = calculate_kb_score_no_shortcuts(
                    kb,
                    freq_list,
                    best,
                    scores_cache,
                ...
                assert full_score > best
                break
        redirect_rank = get_rank(redirect_sum, scores_dict[Categories.REDIRECT])
        score += constants.REDIRECT_WEIGHT * redirect_rank
        sfb_sum = 0
        discomfort_sum = 0
        for bigram in sfb_evaluator.bigrams.items():
            ...
            if (
                score
                + constants.SFB_WEIGHT * get_rank(sfb_sum, scores_dict[Categories.SFB])
                + constants.DISCOMFORT_WEIGHT
        ...
        score += constants.DISCOMFORT_WEIGHT * discomfort_rank
        score += constants.SFB_WEIGHT * sfb_rank
        sfs_sum = 0
        ...
        score += constants.SFS_WEIGHT * sfs_rank

        fingerfreq_sum = finger_freq_evaluator.evaluate_finger_frequencies(
            constants.GOAL_FINGER_FREQ
        )
        fingerfreq_rank = get_rank(fingerfreq_sum, scores_dict[Categories.FINGERFREQ])
        score += constants.FINGER_FREQ_WEIGHT * fingerfreq_rank
        # 0.8 - 0.6 = 0.2
        # x/const.inacc_weight = 0.2
        minimum_failing_inaccuracy_score = get_rank_inverse(
            (best - score) / constants.INACCURACY_WEIGHT,
            scores_dict[Categories.INACCURACY],
        )
        # print("MFI:", minimum_failing_inaccuracy_score)
        testsum = score + constants.INACCURACY_WEIGHT * get_rank(
        ...
        score += constants.INACCURACY_WEIGHT * inaccuracy_rank

        scores_cache[kb] = {
            "score": (score, score),
            "fingerfreq": (fingerfreq_sum, fingerfreq_rank),
            "accuracy": (inaccuracy_sum, inaccuracy_rank),
            "discomfort": (discomfort_sum, discomfort_rank),
            "sfb": (sfb_sum, sfb_rank),
            "sfs": (sfs_sum, sfs_rank),
            "redirect": (redirect_sum, redirect_rank),
        }
...
def calculate_kb_score_no_shortcuts(
    ...
    score += constants.FINGER_FREQ_WEIGHT * fingerfreq_rank
    minimum_failing_inaccuracy_score = get_rank_inverse(
        (best - score) / constants.INACCURACY_WEIGHT, scores_dict[Categories.INACCURACY]
    )
    inaccuracy_sum = evaluate_inaccuracy(kb, freq_list, sys.float_info.max)
    inaccuracy_rank = get_rank(inaccuracy_sum, scores_dict[Categories.INACCURACY])
    score += constants.INACCURACY_WEIGHT * inaccuracy_rank
